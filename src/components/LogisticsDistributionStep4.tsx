    ...
    <div className="space-y-6 text-xs font-semibold text-slate-705">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {crew.map((c) => {
          ...
          return (
            <div key={c.id} className="bg-white border border-slate-150 rounded-2xl p-5 shadow-2xs relative overflow-hidden flex flex-col justify-between space-y-4">
              
              <div className="space-y-3">
                {/* Header detail */}
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <span className="bg-[#11512f]/10 text-[#11512f] px-2.5 py-0.5 rounded-md text-[9px] uppercase tracking-wider font-extrabold">{c.role}</span>
                    <h4 className="text-xs font-black text-slate-800 font-serif leading-tight mt-1">{c.namaKru}</h4>
                    {c.nomorHp && (
                  ...
                  <div className="text-right">
                    ...
                    <span className={`text-[11.5px] font-black font-mono leading-none ${isOverloaded ? 'text-rose-600' : 'text-[#11512f]'}`}>
                ...
                <div className="space-y-1.5">
                  <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden flex shadow-2xs">
                    <div 
                  ...
                  {isOverloaded && (
                    <div className="p-1 px-2.5 bg-rose-50 border border-rose-100 text-rose-700 font-bold rounded-md animate-bounce text-[9px] flex items-center space-x-1">
                      <AlertTriangle className="h-3 w-3 text-rose-600 shrink-0" />
                      <span>WARNING: Kelebihan Kapasitas Bagasi!</span>
                    </div>
                  )}
                </div>

                {/* LIST OF DISTRIBUTED LOGISTICS ITEMS */}
                <div className="pt-2 border-t border-slate-100 space-y-1.5 min-h-[100px] max-h-48 overflow-y-auto pr-1">
                  {stats.items.length > 0 ? (
                    stats.items.map((it) => (
                      <div key={it.distId} className="flex justify-between items-center bg-slate-50/70 p-1.5 px-2 rounded-lg hover:bg-slate-50 border border-slate-100/40 text-[10.5px]">
                        <span className="max-w-[130px] truncate block font-bold text-slate-700">{it.nama}</span>
                        
                        <div className="flex items-center space-x-1.5 font-mono">
                          <span className="text-slate-800 font-black shrink-0">{it.jumlah} pcs</span>
                          <span className="text-[9.5px] text-slate-400 font-bold">({it.beratTotal.toFixed(1)}kg)</span>
                    ...
                    <div className="text-center py-6 text-slate-300 font-normal italic text-[10px]">
                      Kargo bagasi kosong
                    </div>
                  )}
                </div>
              ...
              {/* QUICK FORM ASSIGNER IN CARD FOOTER */}
              <form onSubmit={(e) => handleQuickAddSubmit(c.id, e)} className="pt-2 border-t border-slate-100/80 flex items-center space-x-1.5">
                <select
                  value={currentQuickDist.idBarang}
                  onChange={(e) => handleUpdateQuickSelectField(c.id, 'idBarang', e.target.value)}
                  className="bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-[10.5px] font-semibold text-slate-605 w-full focus:outline-hidden"
                >
                ...
                {currentQuickDist.idBarang && (
                  <>
                    <input
                      ...
                      className="w-8 p-1 text-center bg-slate-50 border border-slate-200 rounded-lg font-mono font-bold text-[10.5px]"
                    />
                    <button
                      type="submit"
                      className="p-1.5 bg-[#11512f] text-white border-0 hover:bg-emerald-800 rounded-lg cursor-pointer"
                    >
